...
public class AccountFactory {
    ...
    public final void setValues(int limit, double commission, double interestOnBalance, HashMap<Integer, Double> interests)  {
    ...
    public final Account createAccount(AccountType account, Client owner, double balance)  {
    ...
    public final Account createAccount(AccountType account, Client owner, double balance, LocalDate expirationDate)  {
        return switch (account) {
            case DEBIT -> new DebitAccount(balance, owner, getInterestOnBalance());
    ...
    public final void setCreditLimit(int limit)  {
        if (limit <= 0) throw new NullOrNegativeBanksException("Limit cannot be null or negative");
        setLimit(limit);
    }

    public final void setCreditComission(double commission)  {
        if (commission <= 0) throw new NullOrNegativeBanksException("Commission cannot be null or negative");
        setCommission(commission);
    }

    public final void setDebitInterest(double interest)  {
        if (interest <= 0)
            throw new NullOrNegativeBanksException("Interest cannot be null or negative");
        setInterestOnBalance(interest);
    }

    public final void setDepositInterests(HashMap<Integer, Double> interests)  {
        if (interests.isEmpty()) throw new NullOrNegativeBanksException("Interests cannot be null");
        setInterests(interests);
    }
}
